...
_服务默认运行在 `http://localhost:3000`。_
...
- **RAG 问答检索**：例如 "用 `rag-search` 找到能回答'我的 RAG 知识库如何组织'的引用片段"。
- **构建上下文包**：例如 "用 `build-note-context` 把 Projects/RAG.md 和今天日记整理成 4000 字以内的回答依据"。
- **检查知识图谱**：例如 "用 `inspect-knowledge-map` 看看 RAG 主题关联了哪些笔记、标签和反链"。
